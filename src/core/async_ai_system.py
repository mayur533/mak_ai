...
class AsyncAISystem(AISystem):
    ...
    async def process_request_async(self, user_input: str) -> Dict[str, Any]:
        ...
        try:
            ...
            context_result = results[1] if not isinstance(results[1], Exception) else None
            tools_result = results[2] if not isinstance(results[2], Exception) else None
            validation_result = results[3] if not isinstance(results[3], Exception) else None
            ...
            plan = await self._generate_plan_async(user_input, analysis_result, context_result)
    ...
    async def _generate_plan_async(self, user_input: str, analysis: Optional[Dict], context: Optional[Dict]) -> Optional[Dict[str, Any]]:
        """Generate plan asynchronously."""
        try:
            # Create prompt
