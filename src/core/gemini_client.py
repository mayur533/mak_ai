...
class IntelligentCache:
    ...
    def set(self, key: str, data: Any, ttl: float = 3600):
        ...
        with self.lock:
            ...
            # Create new entry
            entry = {"data": data, "timestamp": time.time(), "size": size, "ttl": ttl}
            ...
            while (
                ...
                or self.memory_usage > self.max_memory_bytes
            ):
                if not self.cache:
                    break
...
class GeminiClient:
    ...
    def _switch_api_key(self):
        ...
        if self.current_api_key == self.api_key and self.api_key_secondary:
            ...
            self.logger.error(
                "No secondary API key available or already using secondary"
            )
    ...
    def get_api_key_status(self) -> Dict[str, Any]:
        ...
        return {
            "current_key": (
                "primary" if self.current_api_key == self.api_key else "secondary"
            ),
            "primary_available": bool(self.api_key),
            "secondary_available": bool(self.api_key_secondary),
            "current_key_preview": (
                f"{self.current_api_key[:8]}..." if self.current_api_key else "None"
            ),
        }
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                if response.status_code == 403:
                    if (
                        attempt == 0
                        and self.current_api_key == self.api_key
                        and self._has_secondary_key()
                    ):
                        self.logger.warning(
                            "Primary API key failed, switching to secondary"
                        )
                        ...
                        if not self._has_secondary_key():
                            self.logger.error(
                                "Primary API key failed and no secondary key available"
                            )
                        else:
                            self.logger.error(
                                "API key authentication failed for both keys"
                            )
                        return None
                ...
                if response.status_code == 429:
                    if (
                        ...
                        self.logger.warning(
                            "Primary API key rate limited, switching to secondary"
                        )
                        self._switch_api_key()
                        continue
                    else:
                        ...
                        if self._has_secondary_key():
                            self.logger.warning(
                                f"Both API keys rate limited. Waiting {retry_after} seconds before retry..."
                            )
                        else:
                            self.logger.warning(
                                f"Primary API key rate limited and no secondary key. Waiting {retry_after} seconds before retry..."
                            )
                ...
                if 400 <= response.status_code < 500:
                    ...
                    if (
                        ...
                        and self.current_api_key == self.api_key
                        and self._has_secondary_key()
                        and response.status_code in [400, 401, 404]
                    ):
                        self.logger.warning(
                            f"Primary API key failed with {response.status_code}, switching to secondary"
                        )
                        self._switch_api_key()
                        continue
                    else:
                ...
                if response.status_code >= 500:
                    if (
                        ...
                        self.logger.warning(
                            f"Primary API key server error {response.status_code}, switching to secondary"
                        )
                        self._switch_api_key()
                        continue
                    else:
                        ...
                        if self._has_secondary_key():
                            self.logger.warning(
                                f"Both API keys server error {response.status_code}. Waiting {wait_time}s before retry..."
                            )
                        else:
                            self.logger.warning(
                                f"Primary API key server error {response.status_code} and no secondary key. Waiting {wait_time}s before retry..."
                            )
                        if attempt < self.max_retries - 1:
                            ...
                            if self._has_secondary_key():
                                self.logger.error(
                                    "Server error persisted after all retries with both API keys"
                                )
                            else:
                                self.logger.error(
                                    "Server error persisted after all retries with primary API key"
                                )
                            return None
                ...
                try:
                    ...
                    self.logger.debug(
                        f"Response parsed successfully. Keys: {list(json_response.keys())}"
                    )
                ...
                self.logger.error(
                    f"Unexpected error during request (attempt {attempt + 1}): {e}"
                )
                if attempt < self.max_retries - 1:
                    time.sleep(self.rate_limit_delay * (2**attempt))
                else:
    ...
    def generate_text(self, prompt: str, config: GeminiConfig = None) -> Dict[str, Any]:
        ...
        try:
            ...
            if self.cache_enabled:
                cache_key = self.intelligent_cache._generate_key(
                ...
                if cached_result is not None:
                    self.logger.debug(
                        f"Intelligent cache hit for prompt: {prompt[:50]}..."
                    )
            ...
            response = self._make_request(
                f"models/{config.model}:generateContent", data
            )
            ...
            if "candidates" not in response:
                ...
                return {
                    "success": False,
                    "error": "Invalid response format - no candidates",
                }
            ...
            if finish_reason == "SAFETY":
                ...
                return {
                    "success": False,
                    "error": "Content blocked due to recitation policy",
                }
            elif finish_reason == "OTHER":
                return {
                    "success": False,
                    "error": "Content generation failed for unknown reason",
                }
            ...
            self.logger.debug(
                f"Text generation successful. Length: {len(generated_text)} characters"
            )
            return result
    ...
    def generate_with_context_caching(
        self, prompt: str, context_key: str, config: GeminiConfig = None
    ) -> Dict[str, Any]:
        """
    ...
    def generate_with_session(
        self, prompt: str, session_id: str = None, config: GeminiConfig = None
    ) -> Dict[str, Any]:
        """
    ...
    def analyze_image(
        ...
        try:
            ...
            if file_size > max_size:
                return {
                    "success": False,
                    "error": f"Image file too large: {file_size / (1024*1024):.1f}MB (max 20MB)",
                }
            ...
            response = self._make_request(
                "models/gemini-2.0-flash:generateContent", data
            )
            ...
            if finish_reason == "SAFETY":
                return {
                    "success": False,
                    "error": "Image analysis blocked by safety filters",
                }
            elif finish_reason == "RECITATION":
                return {
                    "success": False,
                    "error": "Image analysis blocked due to recitation policy",
                }
            elif finish_reason == "OTHER":
                return {
                    "success": False,
                    "error": "Image analysis failed for unknown reason",
                }
            ...
            self.logger.debug(
                f"Image analysis successful. Result length: {len(analysis_text)} characters"
            )
            return result
    ...
    def generate_structured_output(
        self, prompt: str, schema: Dict[str, Any], config: GeminiConfig = None
    ) -> Dict[str, Any]:
        """
...
"""
        ...
        if response.get("success"):
            try:
                ...
                return {"success": True, "data": structured_data, "raw_text": text}
            except json.JSONDecodeError as e:
                return {
                    "success": False,
    ...
    def get_available_models(self) -> Dict[str, Any]:
        ...
        try:
            response = requests.get(
                f"{self.base_url}/models", headers=self._get_headers(), timeout=10
            )
            response.raise_for_status()
    ...
    def generate_with_google_search(
        self, prompt: str, config: GeminiConfig = None
    ) -> Dict[str, Any]:
        """
        ...
        if response and "candidates" in response:
            ...
            if "content" in candidate:
                ...
                return {
                    ...
                    "web_search_queries": grounding_metadata.get(
                        "webSearchQueries", []
                    ),
                    "grounding_chunks": grounding_metadata.get("groundingChunks", []),
                    "grounding_supports": grounding_metadata.get(
                        "groundingSupports", []
                    ),
                }
    ...
    def get_cache_stats(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "cache_size_mb": sum(len(str(item)) for item in self.context_cache.values())
            / 1024
            / 1024,
        }
    ...
    def enhanced_web_search(self, query: str, adaptive: bool = True) -> Dict[str, Any]:
        ...
        try:
            ...
            self.logger.info(
                f"📊 Query complexity: {complexity['description']} (sources: {complexity['sources']}, tokens: {complexity['max_tokens']})"
            )
            ...
            try:
                ...
                return {
                    "success": False,
                    "error": "Google Search not available - dependencies not installed",
                }
            ...
            try:
                ...
                return {
                    "success": False,
                    "error": f"Failed to configure search client: {str(e)}",
                }
...
Please provide a comprehensive, well-researched answer using the available tools and sources."""
    ...
    def _determine_query_complexity(self, query: str) -> dict:
        ...
        # Count indicators
        simple_count = sum(
            1 for indicator in simple_indicators if indicator in query_lower
        )
        complex_count = sum(
            1 for indicator in complex_indicators if indicator in query_lower
        )
        research_count = sum(
            1 for indicator in research_indicators if indicator in query_lower
        )
        technical_count = sum(
            1 for indicator in technical_indicators if indicator in query_lower
        )
...
Please structure your response clearly for each URL."""
            ...
            self.logger.success(
                f"✅ URL analysis completed in {analysis_time:.2f} seconds"
            )
