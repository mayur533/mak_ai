...
class MetricsCollector:
    ...
    def _calculate_percentiles(self, values: List[float], percentiles: List[int] = [50, 95, 99]) -> List[float]:
    ...
    def get_health_status(self) -> Dict[str, Any]:
        ...
        health_checks = {
            'cpu': {
                'status': 'healthy' if summary['system']['cpu_percent'] < cpu_threshold else 'warning',
                'value': summary['system']['cpu_percent'],
                'threshold': cpu_threshold
            },
            'memory': {
                'status': 'healthy' if summary['system']['memory_percent'] < memory_threshold else 'warning',
                'value': summary['system']['memory_percent'],
                'threshold': memory_threshold
            },
            'disk': {
                'status': 'healthy' if summary['system']['disk_percent'] < disk_threshold else 'warning',
                'value': summary['system']['disk_percent'],
                'threshold': disk_threshold
            },
            'success_rate': {
                'status': 'healthy' if summary['application']['success_rate_percent'] > success_rate_threshold else 'warning',
    ...
    def _export_prometheus_format(self, summary: Dict[str, Any]) -> str:
        ...
        lines.append(f"avg_response_time_seconds {summary['application']['avg_response_time']}")
...
def record_request_metric(duration: float, success: bool, error_type: Optional[str] = None):
    """Record a request metric."""
