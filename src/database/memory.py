...
class ToolManager:
    ...
    def update_tool_usage(self, name: str):
        ...
        if name in self.tools:
            ...
            cursor = self.db.cursor()
            cursor.execute(
                "UPDATE tools SET last_used = ? WHERE name = ?",
                (time.time(), name)
            )
            self.db.commit()
...
class ExecutionHistory:
    ...
    def log_execution(self, step_id: str, step_description: str, 
        ...
        cursor.execute("""
            ...
            VALUES (?, ?, ?, ?, ?, ?)
        """, (str(uuid.uuid4()), step_id, step_description, result, 
              time.time(), error_message))
        self.db.commit()
