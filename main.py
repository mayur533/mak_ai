...
Examples:
  ...
  python main.py --config custom.env # Use custom config file
    ...
    parser.add_argument(
        "--voice", 
        action="store_true", 
        help="Enable voice mode for speech input/output"
    )
    ...
    parser.add_argument(
        "--version", 
        action="version", 
        version="AI Assistant System v1.0.0"
    )
    ...
    if not settings.validate_api_keys():
        logger.error("No valid API keys found. Please set GEMINI_API_KEY or GEMINI_API_KEY_SECONDARY in your .env file.")
